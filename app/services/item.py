...
def get_item(db: Session, order_item: uuid.UUID, product_item: uuid.UUID) -> Item | None:
    stmt = select(Item).where(Item.product_id == product_item, Item.order_id == order_item)
...
def decrease_product_quantity(
    ...
    if product.product_quantity < amount:
        raise NotEnoughError(f"Product with ID: {product.product_id} does not have sufficient quantity.")
